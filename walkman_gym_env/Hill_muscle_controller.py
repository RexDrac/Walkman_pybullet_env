import numpy as np
from walkman_gym_env.filter import FilterClass, EMAFilter, KalmanFilter, KalmanFilterFull

class HillMuscleController:
    def __init__(self, gains = [0,0,0,0], p_rest=0, u_max = 0, v_max = 0, name = None, is_filter = [False, False, False], T = None, cutoff = None, N = None):
        self.Kp = gains[0]
        self.Kd = gains[1]
        self.Kr = gains[2]
        self.Kt = gains[3]
        ...
        # X = [[1,0],[0,1]]
        # self.kalman_test = KalmanFilterFull
    ...
    def control(self, target_position, target_velocity):
        ...
        e_rest = float(self.p_rest-self.measured_position)

        self.u_e = self.Kp*e
        self.u_de = self.Kd*de
        self.u_rest = self.Kr*e_rest
        u = self.u_e + self.u_de + self.u_rest
        u = (u+self.Kt*self.u_past)/(1.0+self.Kt)
        u = np.clip(u, -self.u_max, self.u_max)
        self.u_past = u
        self.u_raw = u
        #
        # # torque calculated using adjusted inputs
        # See "Stable Proportional-Derivative Controllers"
        # https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=5719567
        # resolve numerical integration errors in a simulation
