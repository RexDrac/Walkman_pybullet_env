...
class Train():
    def __init__(self, config):
        ...
        # create new network
        dir_path = 'TRPO/record/' + '3D_fall/' + path_str + datetime.now().strftime('%Y_%m_%d_%H.%M.%S')
    ...
    def get_single_path(self):
        ...
        base_pos_nom = [0,0,1.675]
        state = self.env._reset(Kp=self.config.conf['Kp'], Kd=self.config.conf['Kd'], base_pos_nom=base_pos_nom)
        self.control.reset(w_task=1.0, w_imitation=0.0) #TODO check
    ...
    def test(self):
        ...
        for i in range(self.config.conf['test-num']):#
            base_pos_nom = [0,0,1.675]
            _ = self.env._reset(Kp=self.config.conf['Kp'], Kd=self.config.conf['Kd'], base_pos_nom=base_pos_nom)
            self.control.reset(w_task=1.0,w_imitation=0.0)
