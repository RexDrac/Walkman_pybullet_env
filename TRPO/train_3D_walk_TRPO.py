...
class Train():
    def __init__(self, config):
        ...
        # create new network
        dir_path = 'TRPO/record/' + '3D_walk/' + path_str + datetime.now().strftime('%Y_%m_%d_%H.%M.%S')
    ...
    def get_single_path(self):
        ...
        self.control.reset(w_imitation=0.0,w_task=1.0)
        ...
        # state = self.env._reset(Kp=self.config.conf['Kp'], Kd=self.config.conf['Kd'], q_nom=q_nom, base_orn_nom=base_orn_nom)
        ...
        for step in range(self.max_step_per_train_episode):
            ...
            ref_action = self.ref_motion.ref_motion()

            next_state, reward, terminal, info = self.control.control_step(action, self.force, ref_action, gait_phase)
    ...
    def test(self):
        ...
        for i in range(self.config.conf['test-num']):#
            ...
            for step in range(self.max_step_per_test_episode):
                ...
                gait_phase = self.ref_motion.count / self.ref_motion.dsr_length
                ref_action = self.ref_motion.ref_motion_avg()
                # ref_action = self.ref_motion.ref_motion()

                next_state, reward, done, info = self.control.control_step(action, self.force, ref_action, gait_phase)
