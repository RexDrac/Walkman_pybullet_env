...
class Train():
    def __init__(self, config):
        ...
        self.env = Valkyrie(
            max_time=self.max_time_per_train_episode, renders=True, initial_gap_time=0.1, PD_freq=self.PD_freq,
        ...
        # create new network
        dir_path = 'TRPO/record/' + '3D_imitation/' + path_str + datetime.now().strftime('%Y_%m_%d_%H.%M.%S')
    ...
    def get_single_path(self):
        ...
        self.control.reset(w_imitation=1.0,w_task=0.0)
        ...
        state = self.env._reset(Kp=self.config.conf['Kp'], Kd=self.config.conf['Kd'], q_nom=q_nom, base_orn_nom=base_orn_nom, base_pos_nom=[0,0,1.5],fixed_base=True)
