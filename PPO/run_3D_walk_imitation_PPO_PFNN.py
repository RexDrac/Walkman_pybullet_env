...
class Run():
    ...
    def test(self):
        ...
        for i in range(self.config.conf['test-num']):#
            ...
            # quat = self.ref_motion.euler_to_quat(0, 0.2616, 0)
            _ = self.env._reset(Kp=self.config.conf['Kp'], Kd=self.config.conf['Kd'], base_pos_nom=[0,0,1.175], base_orn_nom=quat, fixed_base=False, base_ang_vel_nom=[0,0,0])
            ...
            # q_dot_nom = self.ref_motion.ref_motion_vel_dict()
            ...
            for step in range(self.max_step_per_episode):
                ...
                self.logging.add_run('phase', gait_phase)
...
def main():
    ...
    dir_path = 'PPO/record/PFNN/3D_walk_imitation/without_external_force_disturbance/2018_11_28_17.40.58'  # '2017_05_29_18.23.49/with_force'
