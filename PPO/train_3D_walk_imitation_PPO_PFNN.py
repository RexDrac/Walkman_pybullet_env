...
from agent.agent_PPO_PFNN import *
...
class Train():
    def __init__(self, config):
        ...
        self.env = Valkyrie(
            ...
            bullet_default_PD=config.conf['bullet-default-PD'], controlled_joints_list=config.conf['controlled-joints'], isEnableSelfCollision=True)
        ...
        # create new network
        dir_path = 'PPO/record/' + 'PFNN/3D_walk_imitation/' + path_str + datetime.now().strftime('%Y_%m_%d_%H.%M.%S')
    ...
    def get_single_path(self):
        ...
        for step in range(self.max_step_per_train_episode):
            ...
            t = time.time()
            next_state, reward, terminal, info = self.control.control_step(action, self.force, next_gait_phase)
    ...
    def train_actor(self, on_policy=True, prefix='', verbose = True): # whether or not on policy
        ...
        if on_policy == True:
            ...
            if self.config.conf['center-advantage']:
                ...
                advantages = np.clip(advantages, -self.config.conf['norm-advantage-clip'], self.config.conf['norm-advantage-clip'])
