...
class Run():
    ...
    def test(self):
        ...
        for i in range(self.config.conf['test-num']):#
            ...
            _ = self.env._reset(Kp=self.config.conf['Kp'], Kd=self.config.conf['Kd'], base_pos_nom=[0,0,1.175], base_orn_nom=quat, fixed_base=False)
            ...
            for step in range(self.max_step_per_episode):
                ...
                # action = mean
                mean = self.agent.agent.action_mean(state)
                action = mean
                ...
                self.logging.add_run('reward_contact', info['reward_contact'])
                #
                # while 1:
...
def main():
    ...
    dir_path = 'PPO/record/MANN/3D_walk_imitation/without_external_force_disturbance/2019_01_20_15.38.09'  # '2017_05_29_18.23.49/with_force'
