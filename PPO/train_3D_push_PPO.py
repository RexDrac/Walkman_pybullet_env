...
class Train():
    def __init__(self, config):
        ...
        self.env = Valkyrie(
            max_time=self.max_time_per_train_episode, renders=False, initial_gap_time=0.5, PD_freq=self.PD_freq,
        ...
        # create new network
        dir_path = 'TRPO/record/' + '3D_push/' + path_str + datetime.now().strftime('%Y_%m_%d_%H.%M.%S')
    ...
    def get_single_path(self):
        ...
        for step in range(self.max_step_per_train_episode):
            ...
            if self.train_external_force_disturbance:
                if step == self.network_freq or step == 6 * self.network_freq or step == 11 * self.network_freq:  # apply force for every 5 second
                    # f = np.random.normal(0.4, 1.0) * 3000#600 * self.network_freq / 10
                    f = np.random.uniform(1200, 3000)
            ...
            next_state, reward, terminal, info = self.control.control_step(action, self.force, np.zeros((len(self.config.conf['controlled-joints']),)))
        ...
        path = dict(observations=np.array(observations), actions=np.array(actions), rewards=np.array(rewards),
                    means=means, logstds=logstds, dones=dones, #actor_infos=actor_infos,
    ...
    def train_paths(self):
        ...
        self.train_critic(True) #TODO train critic before actor?
    ...
    def train_actor(self, on_policy=True, prefix='', verbose = True): # whether or not on policy
        ...
        if on_policy == True:
            ...
            self.agent.train_actor_PPO(observations, actions, advantages, means, logstds)
