...
class TRPO:
    ...
    def setup_actor_training_method(self):
        ...
        self.bound_loss = self.grad_invert.bound_loss(self.action_mu)
        self.surr_loss = self.surr_loss+self.bound_loss
        ...
        if self.config.conf['actor-logstd-grad'] == False:
            temp=[]
            for v in surr_grad:
                v = tf.zeros([self.action_dim], dtype=tf.float32) if v == None else v  # type needs to match
                temp.append(v)
            surr_grad = temp
